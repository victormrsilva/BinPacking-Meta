...
{
    const int nCols = lp_cols(lp);
    char *colType = new char[nCols];

    for(int i = 0; i < nCols; i++)
    {
        if(lp_is_binary(lp, i))
            colType[i] = 1;
        else if(lp_is_integer(lp, i))
            colType[i] = 2;
        else
            colType[i] = 0;
    }
...
char* lp_varName(LinearProgram *lp, int idx){
    char* value;
    #ifdef GRB
        int grbError = GRBgetstrattrelement(lp->lp, "VarName", idx, &value);
        lp_check_for_grb_error( LPgrbDefaultEnv, grbError, __FILE__, __LINE__ );
    #endif
    //printf("%s \n", value);
    return value;
...
void lp_as_integer(LinearProgram *lp){
    #ifdef GRB
        char *vTypes = new char[lp_cols(lp)];
        memset( vTypes, GRB_INTEGER, sizeof(char)*lp_cols(lp) );
        int grbError = GRBsetcharattrarray( lp->lp, "VType", 0, lp_cols(lp), vTypes );
        lp_check_for_grb_error( LPgrbDefaultEnv, grbError, __FILE__, __LINE__ );
        grbError = GRBupdatemodel( lp->lp );
        lp_check_for_grb_error( LPgrbDefaultEnv, grbError, __FILE__, __LINE__ );
        delete[] vTypes;
...
void lp_add_cols( LinearProgram *lp, 
        ...
        vector< string > names )
{
    char **strname = lp_string_vchar(names);
    lp_add_cols( lp, obj.size(), &obj[0], &lb[0], &ub[0], &integer[0], strname );
    delete[]  strname[0];
    delete[]  strname;
}
